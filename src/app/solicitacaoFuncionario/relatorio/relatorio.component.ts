    ...
    const pdf = new jsPDF();
      ...
      if (receitas.length === 0) {
        pdf.text('Nenhuma receita encontrada no período.', 20, y);
          ...
          Object.entries(receitasDia).forEach(([data, valor]) => {
            const valorFormatado = valor.toLocaleString('pt-BR', {
                                                          style: 'currency',
                                                          currency: 'BRL'});

            pdf.text(`${data} - ${valorFormatado}`, 20, y);

            y += 7;
          });

          const total = Object.values(receitasDia).reduce((soma, valor) => soma + valor, 0);
          ...
          pdf.setFont('helvetica', 'bold');
          pdf.text(`TOTAL - ${totalFormatado}`, 20, y);
          pdf.setFont('helvetica', 'normal');
        }
